...
def load_generator(model, checkpoint, device):
    ...
    if missing: print(f'Warning: {len(missing)} parameters missing.')
    if unexpected: print(f'Warning: {len(unexpected)} checkpoint parameters unused.')
    model.eval()
...
def main():
    ...
    model = define_G(3, 3, args.ngf, args.netG, args.norm, use_dropout=False, init_type='normal', init_gain=0.02)
    load_generator(model, args.checkpoint, device)
    model.to(device)
    files = sorted(p for p in Path(args.input_dir).iterdir() if p.is_file() and p.suffix.lower() in {'.tif','.tiff'})
    if not files: raise RuntimeError(f'No TIFF files found in {args.input_dir}')
    for src_path in files:
        ...
        with rasterio.open(dst_path, 'w', **profile) as dst:
            dst.write(output)
            for i, name in enumerate(('Red','Green','Blue'), 1): dst.set_band_description(i, name)
